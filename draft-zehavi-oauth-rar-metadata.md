...
The client proceeds to this step if: (a) no `authorization_reference` was present, (b) no matching token was found in the token bag, (c) a matched token was rejected by the RS (Step 2, item 4), or (d) the client elects to skip the token-bag lookup and use `authorization_details` directly.
...
- If the token is valid but lacks sufficient **authentication context** (e.g., ACR/AMR level), the RS returns `insufficient_user_authentication` per {{RFC9470}}.
...
The `authorization_remediation` parameter carries the actionable `authorization_details` and optional `authorization_reference` as specified in Section 4. The RS constructs these per the rules defined below.
