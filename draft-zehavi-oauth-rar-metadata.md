...
* The `authorization_remediation` error parameter, which contains a base64url-encoded JSON object guiding the client on remediating the error. Its attributes are:
   ...
   * `authorization_reference`: RECOMMENDED. An opaque string generated by the resource server to enable the client to select an existing access token associated with equivalent authorization details, without requiring the client to understand the semantics of the authorization details object. Clients MUST treat this value as opaque and MUST NOT attempt to interpret or derive meaning from it. The value MUST NOT reveal any sensitive or private information. The resource server SHALL NOT include this attribute when tokens issued for the provided `authorization_details` are intended for single-use only.
